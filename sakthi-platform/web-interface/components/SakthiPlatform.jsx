...
const SakthiPlatform = () => {
  ...
  const startProcessing = useCallback(async (intent) => {
  if (uploadedFiles.length === 0) {
    alert('Please upload Oracle SQL files first');
    return;
  }
  ...
  const newSession = {
    id: sessionId,
    name: intent || 'Oracle to BigQuery Conversion',
    status: 'processing',
    progress: 0,
    startTime: new Date().toISOString(),
    files: uploadedFiles.map(f => f.name),
    type: 'oracle_to_bigquery'
  };
  ...
  try {
    for (const file of uploadedFiles) {
      ...
      // Call Sakthi backend API
      const response = await fetch('http://localhost:8000/api/v1/convert/oracle-to-bigquery', {
        method: 'POST',
        body: formData,
      });
      ...
      if (result.success) {
        ...
        // Download BigQuery SQL
        const blob = new Blob([result.bigquery_sql], { type: 'text/sql' });
        ...
        // After successful conversion, also download MCP config if present
        if (result.mcp_config) {
          const mcpBlob = new Blob([result.mcp_config], { type: 'application/json' });
          const mcpUrl = window.URL.createObjectURL(mcpBlob);
          const mcpLink = document.createElement('a');
          mcpLink.href = mcpUrl;
          mcpLink.download = `mcp_config_${Date.now()}.json`;
          document.body.appendChild(mcpLink);
          mcpLink.click();
          window.URL.revokeObjectURL(mcpUrl);
          document.body.removeChild(mcpLink);
        }
    ...
    setProcessingSessions(prev => 
      prev.map(session => 
        session.id === sessionId 
          ? { ...session, status: 'failed', error: error.message }
          : session
      )
    );
